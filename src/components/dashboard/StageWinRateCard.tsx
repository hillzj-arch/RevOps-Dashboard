...
export function StageWinRateCard() {
  ...
  return (
    <div ref={cardRef} className="relative group bg-white rounded-2xl border border-gray-200 p-6 space-y-6">
      ...
      <button onClick={() => cardRef.current && downloadAsPng(cardRef.current, "stage-win-rate")} className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50" title="Download as PNG">
