...
export function PipelineTrendsCard() {
  ...
  const { createdCount, createdAmount } = useMemo(() => calcTrends(deals, region || undefined), [deals, region])

  const countData = createdCount.map((p) => ({ quarter: p.quarter, "Deals Created": p.value }))
  const amountData = createdAmount.map((p) => ({ quarter: p.quarter, "Pipeline Value": p.value }))

  return (
    <div ref={cardRef} className="relative group bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Pipeline Trends</h2>
          <p className="text-xs text-gray-500 mt-0.5">Deals created by quarter</p>
        </div>
        <select
          value={region}
      ...
      <button
        ...
        onClick={() => cardRef.current && downloadAsPng(cardRef.current, "pipeline-trends")}
